...
export default function PacketDisplay() {
  ...
  const [chartData, setChartData] = useState<{ timestamp: string, length: number }[]>([])
  ...
  const updateChartData = (packets: Packet[]) => {
    const newChartData = packets.slice(0, 20).map(packet => ({
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData}>
                ...
                <Legend />
                <Line type="monotone" dataKey="length" stroke="var(--color-length)" name="Packet Length" />
              </LineChart>
            </ResponsiveContainer>
          </ChartContainer>
        </CardContent>
